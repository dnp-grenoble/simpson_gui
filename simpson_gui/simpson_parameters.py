...
def repulsion_scheme(name):
    the_number_part = re.split(r"(\d+)", name)
    print(the_number_part)
    num_angles = np.loadtxt('resources/repangles_num.txt')
    print(num_angles)
    print(np.where(num_angles == int(the_number_part[1]))[0])
